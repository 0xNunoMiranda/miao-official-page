...
const Hero: React.FC<HeroProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      imageLoaded[currentSeasonImage] &&
      (!videoReady[currentSeasonImage] || videoError[currentSeasonImage])
    ) {
      setTimeout(() => {
        if (season === "winter") {
  ...
  useEffect(() => {
    ...
    if (season === "normal" && videoNormal) {
      ...
      videoNormal.addEventListener("error", () => handleVideoError("normal"), {
        once: true,
      });
      if (videoNormal.readyState >= 3) {
        handleVideoCanPlay("normal");
      } else {
        videoNormal.addEventListener(
          "canplaythrough",
          () => handleVideoCanPlay("normal"),
          { once: true }
        );
      ...
      // Adicionar handler de erro
      videoFall.addEventListener("error", () => handleVideoError("fall"), {
        once: true,
      });
      if (videoFall.readyState >= 3) {
        handleVideoCanPlay("fall");
      } else {
        videoFall.addEventListener(
          "canplaythrough",
          () => handleVideoCanPlay("fall"),
          { once: true }
        );
      ...
      // Adicionar handler de erro
      videoWinter.addEventListener("error", () => handleVideoError("winter"), {
        once: true,
      });
      if (videoWinter.readyState >= 3) {
        handleVideoCanPlay("winter");
      } else {
        videoWinter.addEventListener(
          "canplaythrough",
          () => handleVideoCanPlay("winter"),
          { once: true }
        );
    ...
    return () => {
      if (videoNormal) {
        videoNormal.removeEventListener("canplaythrough", () =>
          handleVideoCanPlay("normal")
        );
        videoNormal.removeEventListener("error", () =>
          handleVideoError("normal")
        );
      }
      if (videoFall) {
        videoFall.removeEventListener("canplaythrough", () =>
          handleVideoCanPlay("fall")
        );
        videoFall.removeEventListener("error", () => handleVideoError("fall"));
      }
      if (videoWinter) {
        videoWinter.removeEventListener("canplaythrough", () =>
          handleVideoCanPlay("winter")
        );
        videoWinter.removeEventListener("error", () =>
          handleVideoError("winter")
        );
      }
    };
  }, [season, videoError]);
  ...
  return (
    <section
      id="hero"
      className="relative min-h-screen pt-28 pb-12 flex items-center overflow-x-hidden overflow-y-auto"
          ...
          <div className="order-1 lg:order-2 space-y-8 md:space-y-10 text-center lg:text-center w-full max-w-full px-2 sm:px-0">
            ...
            <div className="flex flex-col items-center gap-4">
              <h1 className="sr-only">
                MIAO - The Green Cat Token - Community Owned Memecoin
              </h1>
              <img
                src="/logo.png"
            ...
            >
              <p
                className={`text-lg md:text-xl lg:text-2xl xl:text-3xl font-black leading-tight ${
              ...
              >
                ...
                <span
                  className="text-[var(--duo-green)] font-black"
                  style={{
                    textShadow: isDark
                      ? "3px 3px 6px rgba(0, 0, 0, 1), -1px -1px 3px rgba(255, 255, 255, 0.5)"
                      : "3px 3px 6px rgba(255, 255, 255, 1), -1px -1px 3px rgba(0, 0, 0, 0.5)",
            ...
            <div className="flex flex-col items-center gap-3 relative z-30">
              ...
              <div
                className="flex flex-nowrap w-full justify-center relative"
                style={{ direction: "ltr" }}
              >
                ...
                <div
                  ...
                  style={{
                    ...
                    boxShadow:
                      "0 6px 10px -3px rgba(0, 0, 0, 0.2), 0 2px 4px -2px rgba(0, 0, 0, 0.1)",
                    direction: "ltr",
                    width: "100%",
                    maxWidth: "100%",
                  }}
              ...
              <div
                className="flex items-center gap-1.5 sm:gap-2 flex-wrap justify-center max-w-full px-2 sm:px-0"
                style={{ direction: "ltr" }}
              >
                <button
